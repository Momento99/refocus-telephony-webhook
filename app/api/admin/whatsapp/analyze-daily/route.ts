...
/**
 * Ежедневный AI-анализ WhatsApp диалогов за вчерашний день (Asia/Bishkek).
 * Использует Claude Haiku 4.5 — самая дешёвая модель Anthropic
 * (~$0.005 за 1 диалог при средней длине).
 *
...
/** Возвращает дату «вчера» в часовом поясе филиалов. */
function yesterdayInBishkek(): string {
  const now = new Date();
  // Получаем YYYY-MM-DD «сейчас» в Asia/Bishkek
  const todayBishkek = new Intl.DateTimeFormat('en-CA', {
    timeZone: TZ, year: 'numeric', month: '2-digit', day: '2-digit',
  }).format(now); // en-CA даёт формат YYYY-MM-DD

  // Вычитаем сутки от полуночи Бишкека
