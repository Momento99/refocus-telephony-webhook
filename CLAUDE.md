...
### Дизайн POS — эталон шапки страницы

ВСЕ страницы POS обязаны использовать единый компонент `PageHeader` из [components/pos/PageHeader.tsx](components/pos/PageHeader.tsx). Не делать собственные шапки, не копировать старые градиентные `h-11 w-11 bg-gradient-to-tr from-teal-400 via-cyan-400 to-sky-400`.
...
<PageHeader
  icon={Receipt}                    // иконка lucide-react по смыслу страницы
  title="Заказы и оплаты"           // короткое название, без описания
  // branchName и terminalCode — опциональны:
  //   - не передавать → автодетект (getBranchConfig + safeGetTerminalCode)
  //   - передать значение → ручное (например, из state страницы)
  //   - передать null → скрыть чип
  actions={                         // правый слот: одна или несколько кнопок
    <button className="…cyan primary…">
...
- **Подзаголовка/описания НЕТ.** Не добавлять «Список заказов филиала», «База клиентов и долги», «Фиксация расходов прямо с кассы» и т.п.
- **Чипы стандартизированы:** `Building2` + `bg-teal-50 text-teal-700 ring-teal-200` для филиала, `Monitor` + `bg-sky-50 text-sky-700 ring-sky-200` для терминала. Менять цвета/иконки нельзя.
- **Иконка страницы:** в cyan-квадрате `h-10 w-10 bg-cyan-500 shadow-[0_4px_20px_rgba(34,211,238,0.40)]`, белая `h-5 w-5`. Подбирать по смыслу (`Receipt`, `Users`, `ReceiptRussianRuble`, `Package`, `Warehouse`, `Clock`, `Layers`).
- **Заголовок:** `text-2xl font-bold tracking-tight text-slate-50` — единый размер на всех страницах.
- **Терминал и филиал** определяются через [lib/terminalCode.ts](lib/terminalCode.ts) (`safeGetTerminalCode`, `normalizeTerminalCode`) и [lib/branchConfig.ts](lib/branchConfig.ts) (`getBranchConfig().branchName`). Не дублировать эти функции внутри страниц.
- **Действия в `actions`:** primary — cyan pill, destructive — rose pill, secondary — `ring-1 ring-slate-700/60 text-slate-300` (т.к. фон страницы тёмный slate-950). Не использовать градиенты `from-teal-400 via-cyan-400 to-sky-400` или розовые тени `shadow-[0_14px_34px_rgba(248,113,113,0.52)]`.

Все остальные правила (карточки, инпуты, таблицы, модалки, кнопки) — по основной CRM design-системе ниже в этом файле. Запрещённое там — запрещено и в POS.
