-- Rename day-12 template reference: aftercare_day12_guarantee_ru → aftercare_day12_warranty_ru
-- The original Meta template was deleted while still PENDING (to update body),
-- but Meta blocks recreation under the same name for several hours after a delete.
-- New template was submitted under aftercare_day12_warranty_ru.
-- This migration updates the RPC to point to the new name.
...
BEGIN
  ...
  -- ─── Day 12 ──────────────────────────────────────────────────────────────
  -- Template renamed to aftercare_day12_warranty_ru (variant B body, no variables).
  WITH eligible AS (
    SELECT
      o.id            AS order_id,
      o.customer_id,
  ...
  SELECT
    d.customer_id, d.order_id, d.branch_id, d.phone_digits,
    'aftercare_day12', 'aftercare_day12_warranty_ru', 'ru',
    '{}'::jsonb,
    now(), 'pending'
  FROM deduped d
  WHERE NOT EXISTS (
