-- ─── Master toggle for automatic WhatsApp customer messages ─────────────────
-- Adds whatsapp_api_config.customer_messaging_enabled (default false).
-- When false, BOTH mark_ready (order_ready) AND enqueue_whatsapp_aftercare
-- (day-3, day-12) skip their work entirely — no rows are inserted into the
-- queue, so nothing gets sent.
--
-- The flag is flipped from the WhatsApp settings admin UI.
-- Intended workflow: keep OFF until Tokmok branch goes live; then user
-- toggles ON and the next daily cron starts firing.

ALTER TABLE public.whatsapp_api_config
  ADD COLUMN IF NOT EXISTS customer_messaging_enabled boolean NOT NULL DEFAULT false;

COMMENT ON COLUMN public.whatsapp_api_config.customer_messaging_enabled IS
  'Master toggle: when false, mark_ready and enqueue_whatsapp_aftercare are no-ops';

-- ─── mark_ready: respect the flag ───────────────────────────────────────────
...
BEGIN
  ...
  -- Master toggle check.
  SELECT COALESCE(customer_messaging_enabled, false)
    INTO v_messaging_enabled
  FROM whatsapp_api_config WHERE id = 1;
  ...
  INSERT INTO whatsapp_followup_queue (
    ...
    scenario, scheduled_at, template_name, template_language,
    template_variables, status, attempts
  ) VALUES (
    v_customer_id, mark_ready.order_id, v_branch_id, v_customer_phone,
    'order_ready', NOW(), 'order_ready_ru', 'ru',
    jsonb_build_object('1', v_first_name, '2', v_branch_name, '3', v_work_hours, '4', v_debt_str),
    'pending', 0
  );
END
$$;

-- ─── enqueue_whatsapp_aftercare: respect the flag ───────────────────────────
...
BEGIN
  ...
  IF NOT v_messaging_enabled THEN
    RETURN jsonb_build_object(
      ...
      'day3_enqueued',  0,
      'day12_enqueued', 0,
      'message',        'customer_messaging_enabled is OFF — toggle in admin UI to start sending'
    );
  END IF;
